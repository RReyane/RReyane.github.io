...
if (MEDIUM_USERNAME) {
  const mediumHandle = MEDIUM_USERNAME.replace(/^@+/, ""); // retire d'éventuels '@'
  const feedUrl = `https://medium.com/feed/@${mediumHandle}`;
  const qs = `/v1/api.json?rss_url=${encodeURIComponent(feedUrl)}`;

  console.log(`Fetching Medium blogs data for @${mediumHandle}`);
  ...
  const req = https.request(options, res => {
    ...
    console.log(`statusCode: ${res.statusCode}`);
    if (res.statusCode !== 200) {
      // ⚠️ corrige le nom de la clé d'erreur :
